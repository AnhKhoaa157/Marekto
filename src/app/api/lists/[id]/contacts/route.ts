...
const SELECT_LIST_CONTACTS_SQL =
  'SELECT c.id, c.workspace_id, c.email, c.first_name, c.last_name, c.phone, c.properties, c.created_at, clr.added_at FROM "Contacts" c INNER JOIN "Contact_list_relation" clr ON clr.contact_id = c.id INNER JOIN "Lists" l ON l.id = clr.list_id WHERE clr.list_id = $1 AND l.workspace_id = $2 ORDER BY clr.added_at DESC, c.id DESC';
const INSERT_LIST_CONTACT_SQL =
  'INSERT INTO "Contact_list_relation" (contact_id, list_id) SELECT $1, $2 WHERE EXISTS (SELECT 1 FROM "Contacts" WHERE id = $1) AND EXISTS (SELECT 1 FROM "Lists" WHERE id = $2 AND workspace_id = $3) ON CONFLICT (contact_id, list_id) DO UPDATE SET added_at = "Contact_list_relation".added_at RETURNING contact_id, list_id, added_at';
const DELETE_LIST_CONTACT_SQL =
  'DELETE FROM "Contact_list_relation" clr USING "Lists" l WHERE clr.list_id = l.id AND clr.contact_id = $1 AND clr.list_id = $2 AND l.workspace_id = $3 RETURNING clr.contact_id, clr.list_id, clr.added_at';
...
function statusForError(message: string): number {
  return ["Invalid workspace id", "Invalid list id", "Invalid contact id"].includes(message)
    ? 400
    : 500;
}
